...
syn keyword	cStatement	GOTO BREAK RETURN CONTINUE NEXT ASM END end
syn keyword	cLabel		CASE SCASE DEFAULT VALUE SVALUE ANY BY AND OR
syn keyword	cConditional	IF ELSE ELSEIF THEN SWITCH start START
syn keyword	cRepeat		WHILE FOR DO UNTIL TO loop LOOP

syn keyword	cTodo		contained TODO FIXME XXX

" A bunch of useful AS keywords
syn keyword	asMove	ABORT HERE JMOVE JM LMOVE DELAY STABLE JAPPRO LAPPRO DRIVE XMOVE LDEPART DRAW DECOMPOSE POINT PO FRAME
syn keyword	asSign	RESET SIGNAL SIG PULSE DLYSIG SCNT SOUT STIM SWAIT TWAIT WAIT ON OFF OF KILL PRINT IFPWPRINT TILL UTIMER PLCAIN MC PRIME DECOMPOSE VAL EXISTREAL PCEXECUTE REPEAT
syn keyword	asState	SPEED ALWAYS ACCURACY FINE ACCEL DECEL PAUSE RUN STOP JAMP SJAMP ERROR SHIFT CALL HOME TASK TRACE QTOOL TOOL ID TRANS EXISTTRANS
syn match	asMove "\#"
syn region	asProgram start=".PROGRA" end="M"
...
syn keyword	cType		INT int LONG long SHORT short CHAR char VOID void
syn keyword	cType		ABS SQRT PI SIN COS ATAN2 RANDOM DX DY DZ
syn keyword	cType		SIGNED signed UNSIGNED unsigned FLOAT float DOUBLE double
...
hi def link asMove		Identifier
hi def link asSign		Type
hi def link asState		Type
hi def link asProgram		Identifier
